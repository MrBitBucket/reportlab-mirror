...
class RL_Codecs:
    ...
    def _256_exception_codec(exceptions):
        import codecs
        decoding_map = codecs.make_identity_dict(xrange(32,256))
        decoding_map.update(exceptions)
        ### Codec APIs
        class Codec(codecs.Codec):
            def encode(self,input,errors='strict', encoding_map = codecs.make_encoding_map(decoding_map)):
                return codecs.charmap_encode(input,errors,encoding_map)

            def decode(self,input,errors='strict', decoding_map=decoding_map):
    ...
    __rl_codecs_cache = {}
    def __rl_codecs(name,cache=__rl_codecs_cache):
        try:
            return cache[name]
        except KeyError:
