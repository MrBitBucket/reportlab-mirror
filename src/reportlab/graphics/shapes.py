...
class Group(Shape):
    ...
    def _explode(self):
        ...
        if hasattr(obj,'transform'): obj.transform = self.transform[:]
        NP = self.namesAndContent()
        while NP:
            name, n = NP.pop(0)
            if isinstance(n, UserNode):
                NP.insert(0,(name,n.provideNode()))
...
class Drawing(Group, Flowable):
    ...
    def _renderPy(self):
        ...
        s += '\tdef __init__(self,width=%s,height=%s,*args,**kw):\n' % (self.width,self.height)
